...
local function s(i, p)
    for k, v in pairs(p) do
        if k == "Font" or k == "TextColor3" or k == "TextSize" then
...
local function cUI(parent, isSub, subTitle, cusTitle)
   ...
   local function addElem(eType, props)
    ...
    local cusHeight = props.CusHeight
    props.CusHeight = nil
    ...
    local cont = addElem("Frame", {CusHeight = 40})
    local sl = c("Frame", {
        Size = UDim2.new(1, -70, 0, 6),
        Position = UDim2.new(0, 35, 0.5, -3),
        Parent = cont,
        BackgroundColor3 = Color3.fromRGB(100, 100, 100)
    })
    local sb = c("TextButton", {
