...
local function s(i, p)
    for k, v in pairs(p) do
        if i[k] ~= nil then
            if (k == "Font" or k == "TextColor3" or k == "TextSize") and (i:IsA("TextLabel") or i:IsA("TextButton") or i:IsA("TextBox")) then
                i[k] = v
            elseif k ~= "Font" and k ~= "TextColor3" and k ~= "TextSize" then
                i[k] = v
            end
        end
    end
...
local function cUI(parent, isSub, subTitle, cusTitle)
    ...
    function ui:Track(label, def, min, max, step, callback)
        ...
        local function update(nv)
            ...
            cont:FindFirstChildOfClass("TextLabel").Text = label .. ": " .. val
        ...
        sl.InputBegan:Connect(function(input)
            if input.UserInputType == Enum.UserInputType.MouseButton1 then
                local relX = input.Position.X - sl.AbsolutePosition.X
                local perc = math.clamp(relX / sl.AbsoluteSize.X, 0, 1)
        ...
        game:GetService("RunService").RenderStepped:Connect(function()
            if drag then
                ...
                local relX = mp.X - sl.AbsolutePosition.X
                local perc = math.clamp(relX / sl.AbsoluteSize.X, 0, 1)
                update(min + (max - min) * perc)
            end
        end)
