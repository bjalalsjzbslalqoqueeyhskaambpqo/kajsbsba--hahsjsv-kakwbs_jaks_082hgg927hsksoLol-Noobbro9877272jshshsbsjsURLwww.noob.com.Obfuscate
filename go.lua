...
function MiniUI.new()
    ...
    function self.createSubMenu(par, txt, parentSubMenu)
        ...
        sb.MouseButton1Click:Connect(function()
            for _, sm in pairs(subMenus) do
                if sm ~= subMenu and sm.ParentSubMenu ~= subMenu then
            ...
            if sf.Visible then
                if parentSubMenu then
                    sf.Position = UDim2.new(1, 5, 0, sb.AbsolutePosition.Y - parentSubMenu.Frame.AbsolutePosition.Y)
                    sf.Parent = parentSubMenu.Frame
                else
                    sf.Position = UDim2.new(0, f.AbsolutePosition.X + f.AbsoluteSize.X + 5, 0, sb.AbsolutePosition.Y)
                    sf.Parent = sg
                end
            end
        end)
        ...
        f:GetPropertyChangedSignal("Position"):Connect(function()
            if sf.Visible and not parentSubMenu then
                sf.Position = UDim2.new(0, f.AbsolutePosition.X + f.AbsoluteSize.X + 5, 0, sb.AbsolutePosition.Y)
            end
        end)
    ...
    function self.createDefaultOptions()
        local _, defaultOptionsContainer = self.addElement("Frame", {CustomHeight = 30, BackgroundTransparency = 1})
        local addDefaultOption, defaultOptionsFrame, defaultOptionsSubMenu = self.createSubMenu(defaultOptionsContainer, "Opciones por defecto")

        local addServerOption, serverOptionsFrame, serverOptionsSubMenu = self.createSubMenu(defaultOptionsFrame, "Server Options", defaultOptionsSubMenu)
        ...
        local addWorldOption, worldOptionsFrame, worldOptionsSubMenu = self.createSubMenu(defaultOptionsFrame, "Worlds", defaultOptionsSubMenu)
