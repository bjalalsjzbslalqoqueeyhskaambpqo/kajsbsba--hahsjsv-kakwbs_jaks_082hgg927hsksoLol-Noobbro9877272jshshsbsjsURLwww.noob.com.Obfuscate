...
local colors = {
    ...
    toggle = Color3.fromRGB(0, 200, 0),
    toggleOff = Color3.fromRGB(100, 100, 100),
    slider = Color3.fromRGB(50, 50, 50),
}

local function createFallingLetters(parent)
    local letters = {}
    local chars = "ABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789"

    for i = 1, 50 do
        local letter = c("TextLabel", {
    ...
    RunService.RenderStepped:Connect(function(dt)
        for _, letter in ipairs(letters) do
            letter.Position = letter.Position + UDim2.new(0, 0, dt * 0.5, 0)
            if letter.Position.Y.Scale > 1 then
                letter.Position = UDim2.new(math.random(), 0, -0.1, 0)
            end
            letter.TextTransparency = math.sin(tick() * 2 + letter.Position.X.Scale * 10) * 0.5 + 0.5
        end
    end)
end
...
local function cUI(parent, isSub, subTitle, cusTitle)
    ...
    local function addElem(eType, props)
        ...
        elseif eType == "TextBox" then
            elem.BackgroundColor3 = colors.foreground
            elem.PlaceholderColor3 = Color3.fromRGB(100, 100, 100)
        end
    ...
    function ui:Track(label, def, min, max, callback)
        ...
        local tl = c("TextLabel", {Size = UDim2.new(1, 0, 0, 20), Position = UDim2.new(0, 0, 0, 0), Text = label .. ": " .. def, Parent = cont, BackgroundTransparency = 1, TextColor3 = colors.text, Font = Enum.Font.Code, TextSize = 14, ZIndex = 10005})
