...
local colors = {
    ...
    toggle = Color3.fromRGB(85, 205, 85),
    toggleOff = Color3.fromRGB(205, 85, 85),
    slider = Color3.fromRGB(85, 85, 95),
}
...
local function cUI(parent, isSub, subTitle, cusTitle)
    ...
    local cb = c("TextButton", {
        ...
        Text = "X",
        Parent = f,
        BackgroundColor3 = Color3.fromRGB(205, 85, 85),
        TextColor3 = colors.text,
        Font = Enum.Font.GothamBold,
        TextSize = 14,
        ZIndex = 10002
    ...
    local function addElem(eType, props)
        ...
        elseif eType == "TextBox" then
            elem.BackgroundColor3 = colors.foreground
            elem.PlaceholderColor3 = Color3.fromRGB(170, 170, 170)
        end
        ...
        if #text > 50 then
            ...
            expandBtn.MouseButton1Click:Connect(function()
                ...
                if expanded then
                    ...
                    expandBtn.Text = "-"
                    local textSize = game:GetService("TextService"):GetTextSize(originalText, txt.TextSize, txt.Font, Vector2.new(txt.AbsoluteSize.X - 10, 1000000))
                    txt.Parent.Size = UDim2.new(1, 0, 0, textSize.Y + 10)
                else
...
function MiniUI:new(cusTitle)
    ...
    local function createIntroAnimation()
        ...
        TS:Create(introFrame, TweenInfo.new(0.5), {BackgroundTransparency = 0}):Play()
        wait(0.5)
        TS:Create(logo, TweenInfo.new(0.5), {TextTransparency = 0}):Play()
        wait(0.5)
        TS:Create(creator, TweenInfo.new(0.5), {TextTransparency = 0}):Play()
        TS:Create(version, TweenInfo.new(0.5), {TextTransparency = 0}):Play()
        wait(1)
        TS:Create(introFrame, TweenInfo.new(0.5), {BackgroundTransparency = 1}):Play()
        TS:Create(logo, TweenInfo.new(0.5), {TextTransparency = 1}):Play()
        TS:Create(creator, TweenInfo.new(0.5), {TextTransparency = 1}):Play()
        TS:Create(version, TweenInfo.new(0.5), {TextTransparency = 1}):Play()
        wait(0.5)
        introFrame:Destroy()
    end
    ...
    function ui:Pass(name, password)
        ...
        local function checkPassword()
            if passBox.Text == password then
                ...
                wait(3)
                ui.mainUI = cUI(sg, false, nil, cusTitle)
                ui:initializeUI()
            else
                passBox.Text = ""
    ...
    function ui:initializeUI()
        ...
        spawn(function()
            wait(0.3)
            local serverSub = ui.mainUI:Sub("Universal Options")
