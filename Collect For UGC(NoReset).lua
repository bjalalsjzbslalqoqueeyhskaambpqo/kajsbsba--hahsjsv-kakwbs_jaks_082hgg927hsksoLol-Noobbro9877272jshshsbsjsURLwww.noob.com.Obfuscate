...
if game.PlaceId == 17103905229 then
    ...
    local speedFilePath = "SpeedData/Speed.txt"

    local function createDirectoryIfNeeded()
        if not isfolder("SpeedData") then
            makefolder("SpeedData")
        end
    end

    local function saveSpeed(speed)
        createDirectoryIfNeeded()
        writefile(speedFilePath, tostring(speed))
    end

    local function loadSpeed()
        createDirectoryIfNeeded()
        if isfile(speedFilePath) then
            return tonumber(readfile(speedFilePath)) or 40
        else
            saveSpeed(40)
            return 40
        end
    ...
    Sec3:CreateButton("Versión 26", sa)
    Sec3:CreateButton("Update: 30/07/24", sa)
...
game:GetService('VirtualUser'):CaptureController()   game:GetService('VirtualUser'):ClickButton2(Vector2.new())
  ...
  spawn(function()
for _, p in game.Players.LocalPlayer.Character:GetChildren() do
